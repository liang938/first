...
void circle_judge()																															//环岛判断
{
	...
	forward_time=(servos_flag==1)?30:35;
	if(time_count==forward_time&&servos_flag_in==0)																					//还没有入环,time_count为入环前直行距离
	{
		P50=1;																																			//关闭蜂鸣器
		time_count=0;																																//时间积分清零
		servos_flag_in=1;																														//开始入环
	}
	if(time_count==70&&servos_flag_in==1&&servos_flag_move==0)										//正在入环，time_count为打死角入环时间
...
void circle_move()																									//环岛寻迹
{
	...
	else																															//入环岛
	{
		if(servos_flag_in==0)																						//如果没有开始入环
			sensor_err = (servos_flag==1)?-10:30;				  								//向相反方向打偏角,防止车身向环岛方向偏移
		else																														//如果开始入环
