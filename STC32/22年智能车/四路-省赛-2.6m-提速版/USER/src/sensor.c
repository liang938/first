...
void Pos_Deviation()	
{
	...
	if(servos_flag!=0)																							//如果是环岛
		...
		sensor_err = 100*(1.7*(l-r)+1.7*(lm-rm))/((l+r)+0.5*(lm+rm));	//差比和算法,偏差值小于零向右转，大于零向左转 
	if(-60<sensor_err&&sensor_err<60)
	{
		speed_add = 50;						//直道加速
	...
	{
		speed_add = (servos_flag==0)?-20:0;						//直道加速
		speed_err	=	600;															//弯道转弯速度增益	
		speed_low = -530;															//转弯差速程度,越小差速越大
		PID_Servos.KP = 1;
		PID_Servos.KI = 0;
		PID_Servos.KD = 35;
	}
